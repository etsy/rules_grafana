...
def json_dashboards(name, srcs):
    ...
    native.filegroup(name=name, srcs=targets)
...
def py_dashboard_builder(src, deps=None):
    if deps == None:
        deps = []
...
def py_dashboards(name, srcs, deps=None):
